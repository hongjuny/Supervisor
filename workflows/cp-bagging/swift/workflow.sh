...
# Pilot 3 Challenge Problem, BAGGING WORKFLOW SH
...
usage()
{
  echo "UNROLLED PARAMETER FILE: usage: workflow.sh SITE EXPID CFG_SYS BAGGING"
}
...
CMD_LINE_ARGS=( -expid=$EXPID
                ...
                -f=$BAGGING # ALW: keeping it as $BAGGING to allow $BAGGING to be a full path
                #-f=$TURBINE_OUTPUT/$BAGGING # Copied to TURBINE_OUTPUT below
              )
